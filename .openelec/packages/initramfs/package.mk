...
case $PROJECT in
  Odroid-U2)  TTY=ttySAC1 ;;
  Odroid-XU3) TTY=ttySAC2 ;;
  Odroid-C1)  TTY=ttyS0   ;;
  Odroid-C2)  TTY=ttyS0   ;;
esac
...
post_install() {
  ...
  if [ -f "$ROOT/.openelec/projects/$PROJECT/initramfs/platform_init" ]; then
    install -m 0755 $ROOT/.openelec/projects/$PROJECT/initramfs/platform_init $ROOT/$BUILD/initramfs/platform_init
  elif [ -f "$ROOT/.openelec/initramfs/platform_init" ]; then
    install -m 0755 $ROOT/.openelec/initramfs/platform_init $ROOT/$BUILD/initramfs/platform_init
  fi
